...
function AsyV2:CreateWindow(opt)
    ...
    local outerFrame = cr("Frame", {
        Parent = sg, 
        Size = UDim2.new(win.size.X.Scale, win.size.X.Offset + 5, win.size.Y.Scale, win.size.Y.Offset + 5),
    ...
    function win:CreateSection(opt)
        local sec = {name = opt.name or "Section", win = self, side = opt.side or "Left"}
